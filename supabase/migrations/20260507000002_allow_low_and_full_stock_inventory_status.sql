...
BEGIN
  ...
  IF constraint_def ILIKE '%''Low''%' AND constraint_def ILIKE '%''Full Stock''%' THEN
    RAISE NOTICE 'Low and Full Stock are already allowed by inventory_status_check; no changes made.';
    RETURN;
  END IF;
  ...
  updated_def := format(
    'CHECK ((%s) OR ((status)::text = ''Low''::text) OR ((status)::text = ''Full Stock''::text))',
    inner_expr
  );
